...
function AllCards() {
    ...
    const isLowRSI = (rsiData) => {
      ...
      return true;
        ...
        if (average > rsiData[rsiData.length - 1] && rsiData[rsiData.length - 1] < 35) {
            return true;
        }
    };

    const allStocks = Object.keys(livePlusIndicator).sort();
    ...
    const getBoughtStocks = (stockName) => {
        ...
        accounts.forEach((acc) => {
            const stockWithValidData = tradeData[acc].find(
                (stock) => stock.stock_name === stockName && stock.quantity && stock.quantity !== 0
            );
